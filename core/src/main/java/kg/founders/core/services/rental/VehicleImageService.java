...
public class VehicleImageService {
    ...
    public List<VehicleImageDto> getImagesByVehicleId(Long vehicleId) {
        return vehicleImageRepository.findByVehicleIdOrderBySortOrderAsc(vehicleId).stream().map(rentalMapper::toVehicleImageDto).collect(Collectors.toList());
    }
    ...
    @Transactional
    public VehicleImageDto addImage(Long vehicleId, String url, Integer sortOrder) {
...
//                .url(url)
        ...
        return rentalMapper.toVehicleImageDto(vehicleImageRepository.save(image));
    }

    @Transactional
    public VehicleImageDto updateImageSortOrder(Long imageId, Integer newSortOrder) {
        VehicleImage image = vehicleImageRepository.findById(imageId)
                .orElseThrow(() -> new EntityNotFoundException("Vehicle's image not found with id: " + imageId));
    ...
    public VehicleImageDto get(Long imageId) {
        return rentalMapper.toVehicleImageDto(vehicleImageRepository.findById(imageId)
                .orElseThrow(() -> new EntityNotFoundException("Vehicle's image not found with id: " + imageId)));
    }
